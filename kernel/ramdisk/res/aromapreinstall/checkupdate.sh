...
then
    cd /sdcard/Download
    for MIUI_VER_NEW in `ls -1 miuiaustralia_zeus-*_full.zip`
    do
        MIUI_VER_NEW_FILE="/sdcard/Download/$MIUI_VER_NEW"
        MIUI_VER_NEW=$(echo $MIUI_VER_NEW | sed 's/miuiaustralia_zeus-//g;s/_full\.zip//g')
    ...
    cd /sdcard/Download
    for MIUI_VER_NEW in `ls -1 miuiaustralia_zeus-*_update.zip`
    do
        MIUI_VER_NEW_FILE="/sdcard/Download/$MIUI_VER_NEW"
        MIUI_VER_NEW=$(echo $MIUI_VER_NEW | sed 's/miuiaustralia_zeus-//g;s/_update\.zip//g')
    done
    if [ `/res/aromapreinstall/updatecomp.sh $MIUI_VER_CURRENT $MIUI_VER_NEW`=="2" ]
    then
        echo "update="$MIUI_VER_NEW_FILE > /cache/update.prop
        echo "version="$MIUI_VER_NEW >> /cache/update.prop
    ...
    if [ -e "/sdcard/Download/miuiaustralia_zeus-"$MIUI_VER_CURRENT"_multilang.zip" ]
    then
        if [ ! -e "/system/etc/multilang" ]
        then
            echo "update=/sdcard/Download/miuiaustralia_zeus-"$MIUI_VER_CURRENT"_multilang.zip" > /cache/update.prop
